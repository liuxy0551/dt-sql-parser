...
lockStatement
    : KW_LOCK KW_TABLE tableOrPartition (KW_SHARED | KW_EXCLUSIVE)
    | KW_UNLOCK KW_TABLE tableOrPartition
    ;

lockDatabase
    : KW_LOCK db_schema dbName=dbSchemaName (KW_SHARED | KW_EXCLUSIVE)
...
createTableStatement
    : KW_CREATE (
        temp=KW_TEMPORARY? trans=KW_TRANSACTIONAL? ext=KW_EXTERNAL? KW_TABLE ifNotExists? name=tableNameCreate (
        ...
        | mgd=KW_MANAGED KW_TABLE ifNotExists? name=tableNameCreate (
            likeTableOrFile rowFormat? tableFileFormat? locationPath? tablePropertiesPrefixed? tableLifecycle?
            | (LPAREN columnNameTypeOrConstraintList RPAREN)? tableComment? createTablePartitionSpec? tableBuckets? tableSkewed? rowFormat?
                tableFileFormat? locationPath? tablePropertiesPrefixed? tableLifecycle? (
                KW_AS selectStatementWithCTE
            )?
